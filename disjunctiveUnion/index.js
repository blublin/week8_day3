...
function symmetricDifferences(numsA, numsB) {
    // If either empty, return other array
    if (!numsA) return [...new Set(numsB)];
    if (!numsB) return [...new Set(numsA)];

    const arr1 = numsA.filter( element => !numsB.includes(element) );
    const arr2 = numsB.filter( element => !numsA.includes(element) );

    let retArr = arr1.concat(arr2)
    // let retArr = [...arr1, ...arr2] // Alternative

    return [...new Set(retArr)]


    // WIP for no-built-ins
    // let aObj = {}, bObj = {};
    // for (let aInd = 0; aInd < numsA.length; aInd++) {
    //     if (aObj[numsA[aInd]]) {
    //         aObj[numsA[aInd]]++
    //     }
    //     else {
    ...
    //     }
    // }
    // for (let bInd = 0; bInd < numsB.length; bInd++) {
    //     if (aObj[numsB[bInd]]) {
    //         aObj[numsB[bInd]]++
    //     }
    //     else {
